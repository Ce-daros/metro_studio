...
- **MapEditor.vue** — 基于 MapLibre 的真实地图编辑器（OSM 瓦片底图、站点点击/拖拽/添加站点/添加线段、AI 添加站点、连续布线、线段曲线渲染、锚点交互、框选、右键菜单、键盘快捷键；选中单站后支持 `PgUp` / `PgDn` 沿线路切换相邻站点，地图网格通过地理图层渲染并随平移/缩放同步；地图加载失败时会输出调试日志）
...
- **StatusBar.vue** — 底部状态栏（战术终端风标签与保存状态指示，展示当前编辑模式、保存状态与工程摘要）
