...
- **SchematicControls.vue** — 示意图视图排版控制菜单（站点显示、线路显示、布局参数配置；支持“骨架展开 / 枢纽分流 / 紧凑程度 / 直线优先 / 标签避让”五轴语义降维控制，以及参数预设的选择、另存、覆盖、删除、导出、导入；修改参数后手动重新排版）
...
- **MenuBar.vue** — 顶部菜单栏（文件/编辑/视图/导出/统计/设置/帮助，含右上角建设年份与工程期数快捷设定，便于新绘制线段自动继承开通年份/分期标签；AI 入口并入编辑/设置菜单；线路切换器与下拉项图标按线路色显示，导航状态缺失时自动降级为空态）
